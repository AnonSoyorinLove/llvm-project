...
static bool isStructLayoutRelocGlobalDecl(CodeGenModule &CGM,
                                           const VarDecl *D,
                                           const RecordDecl **RecordOut) {
  if (!D || !D->isFileVarDecl() || D->getTLSKind())
    return false;
  return isStructLayoutRelocGlobalType(CGM, D->getType(), RecordOut);
}
